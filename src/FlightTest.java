...
public class FlightTest {
	...
	class Trip {
		...
		public int getDuration(){
			int duration = 0;
			if (flights.size() != 0) {
				Time time1 = flights.get(0).getDepartureTime();
				Time time2 = flights.get(flights.size() - 1).getArrivalTime();
				duration = time1.minutesUntil(time2);
			}
			return duration;
		}
		...
		public int getShortestLayover() {
			int layover = -1;
			if (flights.size() >= 2) {
				Time t1 = flights.get(0).getArrivalTime();
				Time t2 = flights.get(1).getDepartureTime();
				layover = t1.minutesUntil(t2);
				for (int i = 0; i < flights.size() - 1; i++) {
					Time time1 = flights.get(i).getArrivalTime();
					Time time2 = flights.get(i + 1).getDepartureTime();
					if (time1.minutesUntil(time2) < layover) {
						layover = time1.minutesUntil(time2);
					}
				}
			}
			return layover;
	...
	public void testGetShortestLayover() throws Exception {
		...
		Flight flight3 = new Flight(new Time("06/09/1978 20:15"), new Time("06/09/1978 23:00"));
		Trip testTrip = new Trip();
